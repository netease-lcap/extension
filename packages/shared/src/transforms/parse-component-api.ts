...
import { babelParser, evalOptions, getSlotName } from '../utils/babel-utils';
import snippetCode2NASL from './snippet-code2nasl';
...
export default function transform(tsCode: string, framework: string): astTypes.ViewComponentDeclaration[] {
  const root = babelParser.parse(tsCode);
          ...
          if (decorator.expression.type === 'CallExpression') {
            ...
            if (calleeName === 'Prop') {
              ...
              if (['EnumSelectSetter', 'CapsulesSetter'].includes(prop?.setter?.concept)) {
                ...
                (prop.setter as any).options = (prop.setter as any).options?.map((option: any, idx) => {
                  if (option.if) {
